...
def suggest_recipes_for_gaps(target, totals, search_fn, max_items=4):
    ...
    # Simple heuristic:
    if need_protein >= 25:
        suggestions += search_fn(
            "high protein quick", min_protein=25, max_calories=None
        )[:max_items]
    elif need_cal >= 300:
        suggestions += search_fn("balanced dinner", min_protein=15, max_calories=600)[
            :max_items
        ]
    else:
        suggestions += search_fn("light snack", min_protein=10, max_calories=300)[
            :max_items
        ]
...
def sync_logged_meals_from_plan(user, day: _date):
    ...
    plans = MealPlan.objects.filter(
        user=user, start_date__gte=start, start_date__lte=start
    )

    # If your ORM/DB stores start_date with timezones, widen the window:
    if not plans.exists():
        plans = MealPlan.objects.filter(
            user=user, start_date__gte=start - timedelta(days=6), start_date__lte=start
        )
